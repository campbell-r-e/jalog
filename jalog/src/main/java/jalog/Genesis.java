...
import jalog.jlog.util.EnhancedFileFilter;
import jalog.jlog.util.ThemeUpdateEventId;
import jalog.util.ScreenUtil;
import jalog.util.UpdateManager;
...
public class Genesis extends JFrame implements ThemeUpdateListener, Debugable {
   ...
   private static final String INI_FILE_NAME  = "jalog.ini" ;
    ...
    */
    private void coreInit() {
      File iniFile = new File(Utility.getAppDataDir(), INI_FILE_NAME);

      if (!iniFile.exists()) {
         try (InputStream in = Genesis.class.getClassLoader().getResourceAsStream(INI_FILE_NAME)) {
            if (in != null) {
               Files.copy(in, iniFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
            } else {
               System.err.println("ERROR: default " + INI_FILE_NAME + " not found in JAR resources.");
            }
         } catch (IOException e) {
            e.printStackTrace();
         }
      ...
      try {
         Utility.load(iniFile.getAbsolutePath(), _optionsDialogBox);
      } catch (IOException e) {
         System.err.println("Could not load " + iniFile.getAbsolutePath() + ": " + e.getMessage());
      }
   }
   ...
   private void quit() {
      ...
      Utility.write(new File(Utility.getAppDataDir(), INI_FILE_NAME).getAbsolutePath(), _optionsDialogBox);
      _notepadFrame.saveText() ;
      System.exit(0);
   }
   ...
   private void postOpenInit() {
      ...
      try {
         if (_optionsDialogBox.isCustomTheme()) {
            ...
            UIManager.setLookAndFeel(_optionsDialogBox.getSelectedTheme()) ;
            SwingUtilities.updateComponentTreeUI(this) ;
         } else {
            System.out.println("Using default FlatLaf Light theme.") ;
         }
      } catch (Exception e) {
         System.out.println("Error loading theme.  Using FlatLaf Light instead.\n") ;
         Debug.logException(e) ;
      }
